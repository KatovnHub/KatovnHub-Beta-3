...
local Window = Rayfield:CreateWindow({
   ...
   ConfigurationSaving = {
      Enabled = false,
      FileName = "KatovnPremium"
   },
   ImageId = "rbxassetid://95214547594099", -- 👑 LOGO PREMIUM
   ImageSize = 32,
   Theme = "AmberGlow"
})
...
TechTab:CreateButton({
   Name = "🔥 Supa Tech V3",
   Callback = function()
      Rayfield:Notify({Title="Info",Content="Coming soon bro 😎",Duration=3})
   end
})
...
FpsTab:CreateButton({
   ...
   Callback = function()
      ...
      Rayfield:Notify({Title="FPS",Content="Boost applied!",Duration=3})
   end
})
...
MoveTab:CreateButton({
   Name = "Load Moveset",
   Callback = function()
      Rayfield:Notify({Title="Moveset",Content="Add script here bro 😏",Duration=3})
   end
})
...
TrollTab:CreateButton({
   Name = "Fake Kick",
   Callback = function()
      Rayfield:Notify({Title="LOL",Content="Just kidding bro 😂",Duration=3})
   end
})
...
TSBTab:CreateButton({
   ...
   Callback = function()
      Rayfield:Notify({Title="TSB",Content="Paste TSB script here",Duration=3})
   end
})
...
AntiTab:CreateButton({
   ...
   Callback = function()
      if AntiAFKEnabled then
         Rayfield:Notify({Title="Anti AFK",Content="Already enabled!",Duration=2})
         return
      end
      AntiAFKEnabled = true
...
DiscordTab:CreateButton({
   ...
   Callback = function()
      ...
      Rayfield:Notify({Title="Success",Content="Discord copied!",Duration=3})
   end,
})
...
SettingTab:CreateDropdown({
   Name = "Change Theme",
   Options = {"AmberGlow","Default","Light","Dark"},
...
task.spawn(function()
   while task.wait(1) do
      pcall(function()
         ...
         infoLabel:Set({
            Title = "📡 Server Info",
            Content = string.format("⚡ Ping: %d ms\n🔥 FPS: %d\n🌍 PlaceId: %d", ping, fps, game.PlaceId)
         })
      end)
   end
end)
